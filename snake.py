...
class Snake:
    ...
    def add_part(self, window: pygame.Surface):
        ...
        if last_part_direction is Snake.UP:
            new_rect["position"] = pygame.Rect(
                last_part.get("position").left,
                last_part.get("position").top + Snake.HEIGHT,
                Snake.WIDTH,
                Snake.HEIGHT,
            )
            ...
            new_rect["position"] = pygame.Rect(
                last_part.get("position").left + Snake.WIDTH,
                last_part.get("position").top,
                Snake.WIDTH,
                Snake.HEIGHT,
            )
    ...
    def move(self, new_direction: str, window: pygame.Surface):
        ...
        for i in range(len(self.body_parts)):
            ...
            if i != 0:
                ...
                part["position"] = self.__move_part(
                    part.get("position"), part.get("current_direction")
                )
                ...
                part["position"] = self.__move_part(
                    front_part.get("position"), new_direction
                )
    ...
    def __move_part(self, previous_part: pygame.Rect, direction: str) -> pygame.Rect:
        ...
        if direction is Snake.UP:
            new_part = pygame.Rect(
                previous_part.left,
                previous_part.top - new_speed,
                previous_part.width,
                previous_part.height,
            )
        elif direction is Snake.DOWN:
            new_part = pygame.Rect(
                previous_part.left,
